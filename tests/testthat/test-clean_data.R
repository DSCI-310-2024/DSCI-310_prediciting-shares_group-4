...
expect_error(clean_data(str_wrong_type), "data.frame")
})

test_that("The data frame should contain all the required columns: shares, num_hrefs, num_imgs, num_videos", 
{expect_error(clean_data(without_shares_df))
 expect_error(clean_data(without_num_hrefs_num_imgs_df))
})


test_that("data_frame shouldn't contain null values", 
{expect_error(clean_data(null_df))})
...
whose values in the `is_popular` column is 0 when `shares` smaller than 1400, otherwise is 0",{
 ...
 expect_equivalent(clean_data(real_data_with_2_rows),real_data_2_rows)
 expect_equivalent(clean_data(real_data_2rows_34),real_data_2rows_34_expected)
})
